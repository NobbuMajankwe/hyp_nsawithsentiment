...
// ── Props — only sidenav collapse state lives here now ───────────────────
interface Props {
  expanded: boolean;
  onToggle: () => void;
}

export function Header({ expanded, onToggle }: Props) {
    ...
    <>
      ...
      <Box
        ...
        sx={{
          ...
          backgroundImage: 'linear-gradient(90deg, rgba(34,211,238,.04), transparent)',
          zIndex: 1200,
      ...
      >
        ...
        <Box
          ...
          sx={{
            width: navWidth - 16, minWidth: navWidth - 16,
        ...
        >
          <Box component="img" src={logo} alt="EventSense AI"
            sx={{ width: 30, height: 30, borderRadius: 2, objectFit: 'cover', flexShrink: 0 }}
          />
          {expanded && (
            <Typography sx={{ fontWeight: 900, color: '#f8fafc', fontFamily: 'monospace', letterSpacing: '-0.5px' }}>
              {'>'} EventSense_AI
            </Typography>
          )}
        </Box>
        ...
        <Chip
          ...
          size="small"
          sx={{
            bgcolor: 'rgba(34,197,94,.08)', color: GREEN,
            border: '1px solid rgba(34,197,94,.22)', fontFamily: 'monospace',
        ...
        <Tooltip title="Account">
          <IconButton onClick={(e) => setMenuAnchor(e.currentTarget)} sx={{ p: 0.5 }}>
            <Avatar sx={{
              width: 34, height: 34, fontWeight: 900, fontFamily: 'monospace',
      ...
      <Box
        ...
        sx={{
          ...
          backgroundImage: 'linear-gradient(rgba(34,211,238,.015), transparent)',
      ...
      >
              ...
              <Tooltip key={item.path} title={!expanded ? item.label : ''} placement="right" arrow>
                <Box
                  ...
                  sx={{
                    ...
                    px: expanded ? 1.5 : 0, py: 1.3,
                    borderRadius: 2, minHeight: 48,
                    cursor: 'pointer',
                    justifyContent: expanded ? 'flex-start' : 'center',
                    bgcolor: active ? 'rgba(34,211,238,.08)' : 'transparent',
                    border: active ? '1px solid rgba(34,211,238,.28)' : '1px solid transparent',
                    '&::before': active ? {
                      content: '""', position: 'absolute',
                      left: 0, top: 6, bottom: 6, width: 3,
                      bgcolor: CYAN, borderRadius: 999, boxShadow: '0 0 18px #22d3ee',
                    } : {},
                    '&:hover': { bgcolor: 'rgba(255,255,255,.04)' },
                  }}
                >
                  <Box sx={{ color: active ? '#a5b4fc' : 'rgba(255,255,255,0.45)', flexShrink: 0, display: 'flex' }}>
                    {item.icon}
                  </Box>
                  {expanded && (
                    <Box sx={{ overflow: 'hidden' }}>
                      <Typography sx={{ color: active ? '#f8fafc' : '#94a3b8', fontFamily: 'monospace', fontWeight: active ? 800 : 500 }}>
                        {item.label}
                      </Typography>
                      <Typography variant="caption" sx={{ color: 'rgba(148,163,184,.5)', fontFamily: 'monospace', fontSize: '.68rem' }}>
                        {item.sublabel}
                      </Typography>
                    </Box>
                  )}
        ...
        <Box sx={{ p: 2, borderTop: `1px solid rgba(34,211,238,.08)` }}>
          <Typography sx={{ color: CYAN, fontSize: '.7rem', fontWeight: 900, mb: 1, fontFamily: 'monospace' }}>
            SYSTEM
          </Typography>
          <Chip label="analytics.online" size="small"
            sx={{ bgcolor: 'rgba(34,211,238,.08)', color: CYAN, fontFamily: 'monospace' }}
          />
        </Box>
      </Box>
    </>
